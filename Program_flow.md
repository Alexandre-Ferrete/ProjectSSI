...
1. Execute `python3 -m server.server --host 0.0.0.0 --port 5555` from `src/`.
2. `ChatServer.__init__()` initializes server configuration, placeholders for storage, user manager, and message router.
3. `ChatServer.start()` creates a listening TCP socket, binds to the host and port, and begins accepting client connections.
4. For each incoming connection, the server creates a client handler thread and continues accepting new clients.

### 2.2 Client startup

1. Execute `python3 -m client.client --host localhost --port 5555` from `src/`.
2. `ChatClient.__init__()` initializes server address, socket state, P2P listener state, and connection maps.
3. `ChatClient.connect()` opens a TCP connection to the server and starts the server receive loop in a new thread.
4. `ChatClient.start_p2p_listener()` opens a second TCP socket on an available port and starts a P2P accept loop thread.
...
3. The client sends the request to the server using length-prefixed JSON over the server socket.
...
2. The client sends an `auth` request to the server.
...
7. The client keeps a TCP connection with the server, listening to another client, waiting for a connection
...
3. Clients can query the server with `get_users` to receive the current list of online users.
...
2. The client sends a `get_ip` request to the server for the recipient username.
3. The server replies with `ip_response`, including the recipient's IP address and port.
4. The client parses the address and calls `connect_to_peer(ip)`.
5. `connect_to_peer()` creates a direct TCP socket to the peer's P2P listener.
6. The client stores the active P2P socket in `p2p_connections` for later messaging.

### 5.2 Receiving peer connections

1. Each client maintains a P2P listener socket opened by `start_p2p_listener()`.
...
3. P2P chat messages are also serialized as JSON and sent over the active peer connection.
...
2. The sender generates an ephemeral keypair and may send the ephemeral public key with the encrypted message.
...
3. Commands that require server state send the appropriate JSON request to the server.
...
5. Whenever an user tries to send a message to another offline user, he will receive an error from the server, saying that that user is offline. So, this user sends the encrypted message to the server, then storage it in a cache.
...
1. Client CLI → `ChatClient` → server via TCP
2. Server → authentication/user management → directory updates
3. Client → server `get_ip` → server returns peer address
4. Client → direct peer connection → encrypted P2P JSON messages
